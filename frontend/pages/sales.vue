...
function statusBadgeColor(s: SaleResponse) {
  const bal = Number(s.balance_amount)
  if (bal <= 0) return 'success'
  if (Number(s.paid_amount) > 0) return 'warning'
  return 'error'
}
...
function statusLabel(s: SaleResponse) {
  ...
  if (Number(s.paid_amount) > 0) return 'Partial'
  return 'Unpaid'
}
  ...
  <div>
    ...
    <UCard class="mb-4">
      <div class="flex flex-wrap items-end gap-3">
        <UInput v-model="filterMonth" type="month" size="sm" label="Month" />
        <USelect v-model="filterParty" :items="[{label: 'All', value: ''}, ...parties.map(p => ({label: p.name, value: p.party_id}))]" size="sm" label="Customer" />
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="s in filteredSales" :key="s.sale_id" class="hover:bg-gray-50 border-b border-gray-200">
              ...
              <td class="p-2">
                <UBadge :color="statusBadgeColor(s)" variant="solid">{{ statusLabel(s) }}</UBadge>
              </td>
              <td class="p-2">{{ formatCurrency(s.grand_total) }}</td>
              <td class="p-2">
                <div class="flex gap-1">
                  <UButton size="sm" variant="ghost" color="primary" @click="viewSale(s)">View</UButton>
        ...
        <div class="md:col-span-7" :class="activeTab === 'preview' ? 'hidden md:block' : ''">
          <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 mb-3">
            ...
            <div class="sm:col-span-3">
              <UInput v-model.number="form.paid_amount" type="number" step="0.01" label="Paid Amount" />
            </div>
            ...
            <table class="w-full">
              ...
              <tbody>
                <tr v-for="(item, i) in form.items" :key="i" class="border-b border-gray-200">
                  ...
                  <td class="p-1">
                    <UInput v-model.number="item.price" type="number" size="sm" class="w-20" @input="calcRow(item)" />
                  </td>
                  <td class="p-1">
                    <UInput v-model.number="item.discount_perc" type="number" size="sm" class="w-14" @input="calcRow(item)" />
                  </td>
                  <td class="p-1 text-sm whitespace-nowrap">{{ formatCurrency(item.discount_amt || 0) }}</td>
                  <td class="p-1">
                    <UInput v-model.number="item.tax_perc" type="number" size="sm" class="w-14" @input="calcRow(item)" />
                  </td>
    ...
    <UModal v-model:open="showView">
      ...
      <div v-if="viewing" class="space-y-2">
        ...
        <p><strong>Status:</strong> <UBadge :color="statusBadgeColor(viewing)" variant="solid">{{ statusLabel(viewing) }}</UBadge></p>
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50">
