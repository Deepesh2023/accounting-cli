...
<template>
  <div>
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <button class="btn btn-success btn-lg" @click="openAdd">+ Record Purchase</button>
    </div>
    ...
    <div class="card">
      <div class="card-body p-0">
        ...
        <div v-else-if="!filteredPurchases.length" class="text-center py-4 text-muted">No purchases found.</div>
        <div v-else class="table-responsive">
          <table class="table table-hover mb-0">
            <thead class="table-light">
              <tr>
                <th>Bill No</th><th>Date</th><th>Supplier</th><th>Status</th><th>Amount</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="s in filteredPurchases" :key="s.purchase_id">
                <td><code>#{{ s.purchase_id?.slice(0, 8) }}</code></td>
                ...
                <td>
                  <button class="btn btn-sm btn-outline-info me-1" @click="viewPurchase(s)">View</button>
                  <button class="btn btn-sm btn-outline-primary me-1" @click="openEdit(s)">Edit</button>
                  <button class="btn btn-sm btn-outline-danger" @click="confirmDelete(s.purchase_id)">Delete</button>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div class="modal-dialog modal-fullscreen">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ editing ? 'Edit' : 'Record' }} Purchase</h5>
          ...
          <div class="modal-body" v-if="activeTab === 'form'">
            <div class="row g-4">
              <div class="col-md-7">
                <div class="row g-3 mb-3">
                  <div class="col-6">
                    ...
                    <div class="input-group">
                      ...
                      <button class="btn btn-outline-secondary" @click="showQuickSupplier = true">+</button>
                    </div>
                  </div>
                  <div class="col-3">
                ...
                <button class="btn btn-sm btn-outline-success" @click="addItem">+ Add Item</button>
                ...
                <div class="row g-3">
                  ...
                  <div class="col-3">
                    <span>Paid: {{ formatCurrency(form.value.paid_amount) }}</span>
                  </div>
                  <div class="col-3">
            ...
            <div v-if="showQuickSupplier" class="mt-3 p-3 border rounded bg-light">
              <h6>Quick Add Supplier</h6>
              <div class="row g-2">
                <div class="col-4"><input v-model="quickSupplierForm.name" class="form-control form-control-sm" placeholder="Name" /></div>
                <div class="col-3"><input v-model="quickSupplierForm.phone" class="form-control form-control-sm" placeholder="Phone" /></div>
                <div class="col-3">
                  <select v-model="quickSupplierForm.state" class="form-select form-select-sm">
                ...
                <div class="col-2">
                  <button class="btn btn-sm btn-success" @click="createQuickSupplier">Add</button>
                </div>
              </div>
            </div>
          </div>
          <div class="modal-footer">
            ...
            <button class="btn btn-success" @click="save">Save Purchase</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showView" class="modal fade show d-block" tabindex="-1">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Purchase #{{ viewing?.purchase_id?.slice(0, 8) }}</h5>
          ...
          <div class="modal-body" v-if="viewing">
            ...
            <table class="table table-sm">
              ...
              <tbody>
                <tr v-for="i in viewing.items" :key="i.purchase_item_id">
