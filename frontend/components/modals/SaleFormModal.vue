...
const form = reactive({
  ...
  taxInclusive: false,
  paidAmount: 0,
  terms: '1. Goods once sold will not be taken back.\n2. Interest @18% will be charged if not paid on time.',
})
...
const partyOptions = computed(() =>
  partiesStore.parties.filter(p => p.type === 'DEBTOR').map(p => ({ label: `${p.name} (₹${p.balance})`, value: p.id }))
)
...
const totals = computed(() => {
  const items = rows.value.map(r => {
    const stockItem = stockStore.get(r.productId)
...
function onProductSelect(row: SaleRow, productId: string) {
...
function openCreate() {
  ...
  rows.value = []
  form.partyId = ''
  form.invoiceNo = `INV-${Date.now().toString(36).toUpperCase()}`
  ...
  form.paidAmount = 0
  form.terms = '1. Goods once sold will not be taken back.\n2. Interest @18% will be charged if not paid on time.'
  isCredit.value = true
...
function submit() {
  const saleItems = totals.value.items.map(item => ({
    id: crypto.randomUUID(),
  ...
  const sale = {
    ...
    partyName: selectedParty.value?.name || 'Walk-in Customer',
    items: saleItems,
  ...
  if (editingId.value) {
    salesStore.update(editingId.value, sale)
  } else {
    salesStore.add(sale)
  }
  open.value = false
...
<template>
  <UModal v-model:open="open" fullscreen :title="editingId ? 'Edit Sale' : 'New Sale'">
    <template #body>
      <div class="flex h-full">
        ...
        <div class="w-1/2 overflow-y-auto p-6 border-r border-gray-200 dark:border-gray-700">
          ...
          <div class="flex items-center gap-4 mb-6">
            <div class="bg-gray-100 dark:bg-gray-800 rounded-lg p-1 flex">
              <button
                :class="['px-4 py-1.5 rounded-md text-sm font-medium transition-colors', isCredit ? 'bg-primary text-white shadow-sm' : 'text-gray-500']"
                @click="isCredit = true"
              >Credit</button>
              <button
                :class="['px-4 py-1.5 rounded-md text-sm font-medium transition-colors', !isCredit ? 'bg-primary text-white shadow-sm' : 'text-gray-500']"
          ...
          <div v-if="activeTab === 'details'" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <UFormField label="Customer">
                <div class="flex gap-2">
                  <USelect v-model="form.partyId" :items="partyOptions" placeholder="Select customer" class="flex-1" />
            ...
            <!-- Invoice meta -->
            <div class="grid grid-cols-2 gap-4">
              <UFormField label="Price Type">
                <USelect v-model="form.taxInclusive" :items="[{ label: 'Tax Exclusive', value: false }, { label: 'Tax Inclusive', value: true }]" />
              </UFormField>
              <UFormField label="Invoice No">
                <UInput v-model="form.invoiceNo" class="w-full" />
              </UFormField>
            ...
            <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-2 text-sm">
              ...
              <div class="flex justify-between text-lg font-bold pt-2 border-t border-gray-200 dark:border-gray-600">
                ...
                <span class="text-primary">₹ {{ totals.grandTotal.toFixed(2) }}</span>
              </div>
        ...
        <div class="w-1/2 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-900">
          <p class="text-xs text-gray-400 uppercase font-semibold mb-4">Invoice Preview</p>
          <InvoicePreview
            type="sale"
            :invoice-no="form.invoiceNo"
            :date="form.date"
            :due-date="form.dueDate"
            :party-name="selectedParty?.name || 'Walk-in Customer'"
    ...
    <template #footer>
      <div class="flex justify-between w-full">
        ...
        <UButton label="Save Sale" color="primary" @click="submit" />
      </div>
    </template>
  </UModal>
</template>
