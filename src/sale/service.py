...
def show_menu(inventory_repository: InventoryRepository, sale_repository: SaleRepository):
    while True:
...
def record_sale_workflow(inventory_repository: InventoryRepository, sale_repository: SaleRepository):
    ...
    while True:
        ...
        if choice == "1":
            ...
            if item:
                ...
                existing = next((si for si in selling_items if si.product_id == item.product_id and si.selling_price == item.selling_price), None)
                if existing:
...
def add_item_to_list(inventory_repository: InventoryRepository) -> Optional[SaleItem]:
    ...
    price = get_input(f"Selling Price [{product.selling_price}]: ", float) or product.selling_price
...
def edit_item_in_list(selling_items: List[SaleItem], inventory_repository: InventoryRepository):
    if not selling_items:
        print("List is empty.")
        return
    ...
    price = get_input(f"New Price [{item.selling_price}]: ", float) or item.selling_price
...
def confirm_sale(inventory_repository: InventoryRepository, sale_repository: SaleRepository, items: List[SaleItem]):
    # 1. Update Inventory
    for item in items:
        product = inventory_repository.get_product(item.product_id)
        if product:
...
def display_selling_items(items: List[SaleItem]):
    ...
    for item in items:
        ...
        print(f"{item.name:<20} {item.selling_price:<10.2f} {item.quantity:<10} {subtotal:<10.2f}")
